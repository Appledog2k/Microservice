...
namespace repoitem.Services
{
    public class RabbitMQServices : BackgroundService
    {
        ...
        public RabbitMQServices(IServiceScopeFactory _serviceScopeFactory)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += (ch, ea) =>
            {
                ...
                var repoItems = JsonConvert.DeserializeObject<RepoItem>(message);

                //Stored Offer Details into the Database
                using (var scope = serviceScopeFactory.CreateScope())
                {
                    var _dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                    var result = _dbContext.RepoItems.Add(repoItems);
